...
import { allowAdmin, authorize } from "../../_middleware/authorize";
...
router.patch('/update', authorize(), allowAdmin(), async function _update(req: Request, res: Response, next: NextFunction) {
...
router.delete('/delete', authorize(), allowAdmin(), async function _delete(req: Request, res: Response, next: NextFunction) {
  try {
    await userService._delete(parseInt(req.query.id as string));
...
router.get('/all', authorize(), async (req: Request, res: Response, next: NextFunction) => {
  try {
