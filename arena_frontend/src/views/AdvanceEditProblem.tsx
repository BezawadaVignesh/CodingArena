  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setContestId(problemData?.contestId)
  ...
  return (
    <div>
      ...
      <div
        ...
        >
        <Typography sx={{ width: "max-content", margin: "auto", fontWeight: 600, fontSize: "25px" }} gutterBottom>
          Advance Settings
        </Typography>
        <div style={{width: "90%", margin: "auto", maxWidth: "1000px",}}>

        <Breadcrumbs sx={{width: "max-content"}}>
        <Link underline="hover"
          color="inherit" href={`/admin/problems/${contestId}`}>
        {problemTitle}
        </Link>
      ...
      >
        ...
        > */}
          {/* <Typography sx={{ fontWeight: 500, fontSize: "20px" }} gutterBottom>
            {problemTitle}
          </Typography> */}
        {/* </div> */}
        <div
        ...
        >
          ...
          <Autocomplete
            ...
            onChange={(_, newValue) => {
              const uniqueLanguages = newValue.filter(
                (v, i, arr) => arr.findIndex((item) => item[0] === v[0]) === i
              );
        ...
        {languages.length === 0 ? (
          ...
          languages.map((lang) => (
            <StarterCode
              key={lang[0]}
              language={lang[0]}
              initialCode={scode[lang[0]] || ""}
              onCodeChange={handleCodeChange}
            />
          ))
        )}
        <Box
          sx={{ marginBlock: "10px", display: "flex", justifyContent: "flex-end" }}
        >
          <CartoonButton variant="success" onClick={handleSave}>
            Save
          </CartoonButton>
        </Box>
        </div>
