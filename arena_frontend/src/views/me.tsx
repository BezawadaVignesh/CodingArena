...
import { Accordion, AccordionDetails, AccordionSummary, Box, Button, Card, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, InputLabel, List, ListItem, ListItemButton, ListItemIcon, ListItemText, MenuItem, Select, Slide, TextField, Typography, useMediaQuery, useTheme } from "@mui/material";
...
import React, { ChangeEvent, FormEvent, memo, useContext, useEffect, useState } from "react";
...
import { StyledTab, StyledTabs } from "./Code";
import style from "./Problem.module.css";


const ContestDetails = memo(({ id, hidden ,reload,setReload}: { id: number; hidden: boolean ;reload:boolean;setReload:React.Dispatch<React.SetStateAction<boolean>>;}) => {
    ...
    const handleSubmit = async () => {
      ...
      return alert?.showAlert("details update successfully","success")
        ...
        return alert?.showAlert("end time should be larger then start time", "error");
      }
    //   console.log(data);
    //   console.log("submit");
    };
    ...
    return (
        ...
        >
            ...
            <div style={{display:"flex"}}>
                ...
                >
                    {/* onClick={handleSave} */}
                    <Button  onClick={()=>handleSave()} variant="contained"  sx={{marginBottom:"10px",marginTop:"10px", boxShadow: 3, '&:hover': {bgcolor: 'blue', color: 'white',  boxShadow: 6,},}}>
      ...
      {qs.map(
          ...
          {qs.length === 0 && (
            <Box display="flex" justifyContent="center" alignItems="center" sx={{ height: '100px' }}>
              <Typography sx={{ fontWeight: 350, fontSize: '20px' }} gutterBottom>
                No Problems added
              </Typography>
            </Box>
          )}
...
const Contest = () => {
    ...
    const [startTime, setStartTime] = useState(dayjs());
    const [endTime, setEndTime] = useState(dayjs());
    // const [created,setMode]= useState<string>("");
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <div style={{width :"100%"}}>
                <div style={{width:'max-width', marginInline:'auto', marginTop:'20px',display:"flex",
                            justifyContent: "space-between",
                            alignItems: "center",
                            ...
                            paddingTop: 25,}}>
                        <div>
                            <Typography  sx={{fontWeight:500, fontSize: '30px' ,padding:0}} gutterBottom >{title}</Typography>
                            <Typography  sx={{fontWeight:400, fontSize: '15px' ,padding:0}} gutterBottom >{mode}</Typography>
                            <Typography  sx={{fontWeight:300, fontSize: '10px'}} gutterBottom >{startTime.format('YYYY-MM-DD HH:mm')}</Typography>
                            <Typography  sx={{fontWeight:300, fontSize: '10px'}} gutterBottom >{endTime.format('YYYY-MM-DD HH:mm')}</Typography>
                ...
                <ContestDetails id={contestId} hidden={tab!=1} reload={reload} setReload={setReload}/>
                <div hidden={tab != 2}>
